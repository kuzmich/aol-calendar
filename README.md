...
Сайт запускается комндой `uv run flask run --debug`. Статический генератор делает html
файлы в папке `out` (`out/2025.html`, `out/2026.html`). Статические страницы публикуются в
интернете с помощью обычного `git push` - Github Action берет файлы из папки `out` и
публикует их в Github Pages.
...
- Где-то вести список учителей, мест и курсов, которых еще нет (пока в коде)

## TODO публичное
- Ссылка на курс/кнопка "Регистрация" в инфо-боксе
- Синхронизация курсов с админкой
